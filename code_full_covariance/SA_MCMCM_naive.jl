...
function step_propose(atoms::Atoms, p::ContinuousMultivariateDistribution, args)
    ...
    q = construct_mixture_distribution(mu, cov_matrix, args)
    theta_new = rand(q)
    return theta_new
end
...
function construct_mixture_distribution(mu, cov_matrix, args)
    ...
    for i in 1:length(args["mixture_coef"])
        push!(mixture_params, (mu, args["mixture_coef"][i]*cov_matrix))
    end
